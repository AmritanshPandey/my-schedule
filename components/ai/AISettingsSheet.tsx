...
import { IconCheck, IconCpu, IconEye, IconEyeOff, IconShield, IconSparkles, IconX } from "@tabler/icons-react";
...
const PROVIDER_OPTIONS: { value: AIProviderType; label: string; note: string }[] = [
  ...
  { value: "browser", label: "Browser", note: "On-device · free" },
];
    ...
    <div className="grid grid-cols-4 gap-1.5 px-1">
      {PROVIDER_OPTIONS.map((option) => (
        ...
        >
          ...
          <span className={`mt-0.5 block text-[9px] ${value === option.value ? "text-white/65 dark:text-neutral-600" : "text-neutral-400"}`}>{option.note}</span>
        </button>
      ))}
    </div>
  );
...
function ProviderCard({ provider }: { provider: AIProviderType }) {
  ...
  const [browserModel, setBrowserModelState] = useState(getBrowserModel);
  ...
  async function testConnection() {
    ...
    setStatus("testing");
    if (provider === "browser") {
      // Pre-warm triggers the download; the status bar shows progress.
      prewarmBrowserAI();
    }
  ...
  const description =
    provider === "mlx" ? "Runs locally through mlx_lm.server."
    : provider === "ollama" ? "Runs locally through the Ollama app."
    : provider === "browser" ? "Runs a small model directly in your browser using WebGPU (GPU) or WASM (CPU). The model is downloaded once and cached — no server or API key required."
  ...
  return (
    <Card>
      <div className="flex items-center gap-3 px-4 py-3.5">
        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-neutral-900 dark:bg-white">{iconEl}</div>
        <div className="min-w-0 flex-1">
          <p className="text-[13px] font-bold text-neutral-900 dark:text-white">{title}</p>
          <p className={`truncate text-[11px] font-medium ${status === "idle" || status === "testing" ? "text-neutral-400" : status.ok ? "text-emerald-600 dark:text-emerald-400" : "text-rose-500"}`}>{statusLabel}</p>
        </div>
        ...
        {provider === "browser" && (
          <>
            <Field
              ...
              onChange={(value) => save(setBrowserModelState, setBrowserModel, value)}
            />
      ...
      <div className="flex items-start gap-2.5 px-4 py-3.5">
        ...
        <p className="text-[11px] leading-relaxed text-neutral-500 dark:text-neutral-400">
          {description}{" "}
          {provider === "api" ? "Your key is stored locally in this browser and sent only to the endpoint you enter." : "No PlanR account or external AI key is required."}
        </p>
      </div>
    </Card>
  );
